...
-   Fork then clone [this repo](https://github.com/eds-221-programming-essentials/eds221-day8-comp) to create a version-controlled R Project for Day 8
...
#| echo: false
# INSTRUCTOR VERSION
lizards <- read_csv(here("course-materials", "interactive-sessions", "data", "lizards.csv"))
```
...
## 4. Advanced `ggplot2` customization
...
#| echo: false
# INSTRUCTOR VERSION
mono <- read_csv(here("course-materials", "interactive-sessions", "data", "mono.csv"))
```
...
# INSTRUCTOR VERSION
# First, read in the Jornada Basin vegetation data: 
jornada_veg <- read_sf(here("course-materials","interactive-sessions", "data","spatial_vegetation","doc.kml")) %>% dplyr::select(Name) %>% 
  clean_names()
```
